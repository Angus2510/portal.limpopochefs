...
import { useGetGuardianDashboardQuery } from '@/lib/features/guardian/guardianApiSlice';

const GuardianTimesheet = ({ guardianId }) => {
  const [currentWeekStart, setCurrentWeekStart] = useState(startOfWeek(new Date(), { weekStartsOn: 1 }));
  const [maxWeekStart] = useState(addWeeks(startOfWeek(new Date(), { weekStartsOn: 1 }), 1));
  const [filteredEvents, setFilteredEvents] = useState([]);

  const { data: fetchedEvents = [], isLoading, isError, error } = useGetGuardianDashboardQuery(guardianId);
...
export default GuardianTimesheet;
