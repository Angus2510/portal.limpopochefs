...
import { useGetAttendanceByMonthQuery } from '@/lib/features/attendance/attendanceApiSlice';
...
const Calendar = ({ studentId }) => {
    ...
    const params = { studentId, year: currentYear, month: currentMonth };
    const { data: attendanceData, error, isLoading } = useGetAttendanceByMonthQuery(params);
    ...
    useEffect(() => {
        if (attendanceData) {
            ...
            attendanceData.ids.forEach((id) => {
                const record = attendanceData.entities[id];
